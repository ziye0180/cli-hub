...
fn sync_codex_provider_writes_auth_and_config() {
    ...
    let auth_path = cli_hub_lib::get_codex_auth_path();
    let config_path = cli_hub_lib::get_codex_config_path();
...
fn sync_enabled_to_codex_writes_enabled_servers() {
    ...
    cli_hub_lib::sync_enabled_to_codex(&config).expect("sync codex");

    let path = cli_hub_lib::get_codex_config_path();
...
fn sync_enabled_to_codex_returns_error_on_invalid_toml() {
    ...
    let err = cli_hub_lib::sync_enabled_to_codex(&config).expect_err("sync should fail");
    match err {
        cli_hub_lib::AppError::Toml { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "path should reference config.toml"
            );
        }
        cli_hub_lib::AppError::McpValidation(msg) => {
            assert!(
...
fn sync_codex_provider_missing_auth_returns_error() {
    ...
    match err {
        cli_hub_lib::AppError::Config(msg) => {
    ...
    // 确认未产生任何 live 配置文件
    assert!(
        !cli_hub_lib::get_codex_auth_path().exists(),
        "auth.json should not be created on failure"
    );
    assert!(
        !cli_hub_lib::get_codex_config_path().exists(),
...
"#;
    ...
    cli_hub_lib::write_codex_live_atomic(&auth, Some(config_text))
    ...
    let stored_auth: serde_json::Value =
        cli_hub_lib::read_json_file(&auth_path).expect("read auth");
...
"#;
    ...
    let err = cli_hub_lib::write_codex_live_atomic(&auth, Some(config_text))
        .expect_err("config write should fail when target is directory");
    match err {
        cli_hub_lib::AppError::Io { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "io error path should point to config.toml"
            );
        }
        cli_hub_lib::AppError::IoContext { context, .. } => {
            assert!(
...
"#,
    ...
    let mut config = MultiAppConfig::default();
    let changed = cli_hub_lib::import_from_codex(&mut config).expect("import codex");
...
fn sync_claude_enabled_mcp_projects_to_user_config() {
    ...
    cli_hub_lib::sync_enabled_to_claude(&config).expect("sync Claude MCP");

    let claude_path = cli_hub_lib::get_claude_mcp_path();
...
fn import_from_claude_merges_into_config() {
    ...
    let changed = cli_hub_lib::import_from_claude(&mut config).expect("import from claude");
...
fn create_backup_skips_missing_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_path = home.join(".cli-hub").join("config.json");
...
fn create_backup_generates_snapshot_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_dir = home.join(".cli-hub");
...
fn sync_gemini_google_official_sets_oauth_security() {
    ...
    let cc_settings = home.join(".cli-hub").join("settings.json");
    assert!(
        cc_settings.exists(),
        "app settings should exist at {}",
        cc_settings.display()
    );
    let cc_raw = std::fs::read_to_string(&cc_settings).expect("read .cli-hub settings");
